...
if __name__ == "__main__":
    ...
    try:
        ...
        visual_model = MicroPizzaNet(num_classes=6).to(device)
        spatial_model = MicroPizzaNetWithSE(num_classes=6).to(device) # Using SE as "spatial" proxy
        
        # Create dummy input for testing (but processing is real)
        dummy_img = torch.randn(1, 3, 48, 48).to(device)
        
        @profiler.profile_encoder("visual_encoder", "MicroPizzaNet")
        def real_visual_encoder(image_tensor):
            with torch.no_grad():
                # Extract features (using the part before classifier)
                x = visual_model.block1(image_tensor)
                x = visual_model.block2(x)
                x = visual_model.global_pool(x)
                return x.flatten(1)
        
        @profiler.profile_encoder("spatial_encoder", "MicroPizzaNetWithSE")
        def real_spatial_encoder(image_tensor):
            with torch.no_grad():
                # Extract features
                x = spatial_model.block1(image_tensor)
                if spatial_model.use_se: x = spatial_model.se1(x)
                x = spatial_model.block2(image_tensor) # Note: Input should be x, but block2 expects 8 channels? 
                # Wait, block2 expects 8 channels. block1 outputs 8.
                # Let's fix the flow:
                x1 = spatial_model.block1(image_tensor)
                if spatial_model.use_se: x1 = spatial_model.se1(x1)
                x2 = spatial_model.block2(x1)
                if spatial_model.use_se: x2 = spatial_model.se2(x2)
                x3 = spatial_model.global_pool(x2)
                return x3.flatten(1)
        
        @profiler.profile_connector("feature_fusion")
        def real_connector(visual_features, spatial_features):
            # Simple concatenation fusion
            return torch.cat([visual_features, spatial_features], dim=1)
        
        print("🔧 Running dual-encoder profiler with REAL models...")
        
        for i in range(5):
            v_feat = real_visual_encoder(dummy_img)
            s_feat = real_spatial_encoder(dummy_img)
            fused = real_connector(v_feat, s_feat)
            
    except ImportError as e:
        print(f"Failed to import real models: {e}")
        print("Please ensure src.pizza_detector is available.")
    except Exception as e:
