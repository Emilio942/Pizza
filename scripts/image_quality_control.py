...
2. Generates an HTML report with thumbnail views
...
def collect_image_paths(image_dirs, exclude_paths=None):
    ...
    Args:
        ...
        exclude_paths: Set of paths to exclude (optional)
...
def select_random_samples(image_paths, num_samples):
    ...
    Args:
        image_paths: Dictionary with class names as keys and lists of image paths as values
        num_samples: Number of samples to select from each category
        
    Returns:
        Dictionary with class names as keys and lists of selected image paths as values
    """
    selected_samples = {}
...
def generate_thumbnails(selected_samples, output_dir):
    ...
    Args:
        selected_samples: Dictionary with class names as keys and lists of selected image paths as values
        ...
        for i, img_path in enumerate(paths):
            try:
                ...
                with Image.open(img_path) as img:
                    # Create a copy to avoid modifying the original during resize
                    img_copy = img.copy()
...
def generate_html_report(selected_samples, thumbnail_map, output_dir, run_id):
    ...
    Args:
        selected_samples: Dictionary with class names as keys and lists of selected image paths as values
        thumbnail_map: Dictionary mapping original paths to thumbnail paths
        output_dir: Directory to save the HTML report
        run_id: Unique identifier for this quality control run
...
"""
    ...
    html_content = html_content.replace("{JSON_DATA_PLACEHOLDER}", js_data_str)
    
    with open(html_path, 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    # Create a simple Python cgi script to handle form submission
...
<body>
    <div class="container">
        ...
        <pre>python scripts/image_quality_control.py --process-marked --output {os.path.dirname(os.path.abspath(output_file))}</pre>
...
def process_marked_images(output_dir):
    """
    Process previously marked images by deleting them from the dataset.
    
    Args:
        output_dir: Directory containing the marked images data
        
    Returns:
        tuple: (number of deleted images, list of deleted paths)
    """
...
def main():
    ...
    image_dirs = [os.path.join(project_root, d) for d in args.image_dirs]
    ...
    if args.process_marked:
        deleted_count, deleted_paths = process_marked_images(output_dir)
        if deleted_count > 0:
            print(f"\nSuccessfully deleted {deleted_count} marked images.")
    ...
    print(f"   python scripts/image_quality_control.py --process-marked --output {args.output}")
