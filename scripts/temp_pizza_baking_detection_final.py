...
import copy
import struct
from src.augmentation.cheese_augment import AddCheese
...
class PizzaDatasetAnalysis:
    ...
    def analyze(self, sample_size=None):
        ...
        logger.info(f"Gefundene Klassenverzeichnisse: {self.class_dirs}")
        ...
        logger.info(f"Klassenzählungen nach Bildersammlung: {self.stats['class_counts']}")
    ...
    def get_preprocessing_parameters(self):
        # Rufe analyze() auf, wenn die Analyse noch nicht durchgeführt wurde
...
def create_optimized_dataloaders(config, preprocessing_params=None):
    ...
    train_transform = transforms.Compose([
        ...
        transforms.RandomRotation(30),
        transforms.RandomAffine(degrees=0, translate=(0.1, 0.1), scale=(0.9, 1.1)),
        AddCheese(cheese_renders_path='data/generated_cheese_renders/', p=0.7), # Hinzugefügt für 3D-Käse-Augmentierung
        transforms.ToTensor(),
        transforms.Normalize(mean=mean, std=std)
    ])
