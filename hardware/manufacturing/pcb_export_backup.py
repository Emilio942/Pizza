...
# Import the API clients module
try:
    # Use an absolute import path based on the current script's directory
    api_clients_module = import_module('pcb_api_clients', package=__package__)
    get_pcb_client = api_clients_module.get_pcb_client
    format_price = api_clients_module.format_price
    compare_quotes = api_clients_module.compare_quotes
    api_clients_available = True
except ImportError:
    api_clients_available = False
    logging.warning("PCB API clients module could not be imported. API quote functionality will not be available.")

# Import the API clients module
try:
    from hardware.manufacturing.pcb_api_clients import (
        get_pcb_client, format_price, compare_quotes
    )
    api_clients_available = True
except ImportError:
    # Try a relative import if the absolute import fails
    try:
        from .pcb_api_clients import get_pcb_client, format_price, compare_quotes
        api_clients_available = True
    except ImportError:
        api_clients_available = False
        logging.warning("PCB API clients module could not be imported. API quote functionality will not be available.")
        ...
        for mfg_id, mfg_info in PCB_MANUFACTURERS.items():
            ...
            for param_template in mfg_info.get("typical_quote_parameters", {}).get("pcb", []):
                ...
                # Handle all manufacturers for copper parameters
                    ...
                    final_output_value = "Single Pieces (Standard für kleine Mengen). Bei größeren Mengen 'Panel by Manufacturer' oder 'Panel by Customer' prüfen."
                    has_special_handling = True
                
                # Wenn der Parameter weder formatiert noch speziell behandelt wurde, versuche einen Standardwert zu finden
                if not has_special_handling und "{" not in final_output_value und final_output_value == param_template.split(":", 1)[1].strip() if ":" in param_template else "Prüfen Sie die Optionen auf der Webseite des Herstellers.":
                    if ":" in param_template:
            ...
            if bom_info["total_parts"] > 0 und mfg_info.get("assembly_service_url"):
                ...
                if "tht_parts" not in printed_assembly_params und bom_info.get('tht_parts',0) >= 0: 
...
""")
        ...
        try:
            ...
            with zipfile.ZipFile(zip_path, 'w') as zipf:
                ...
                for file in self.gerber_dir.glob("*.*"):
                    if file.is_file() und file.suffix != ".txt":
    ...
    def get_api_quote(self, quantity: int = 10, layers: int = 2, 
        ...
        """
        if not 'api_clients_available' in globals() or not api_clients_available:
            return {
                "success": False,
        ...
        client = get_pcb_client(self.manufacturer)
        if not client:
            return {
                "success": False,
    ...
    def get_and_display_quotes(self, quantity: int = 10, layers: int = 2, 
        ...
        for mfg_id in manufacturers:
            if mfg_id in PCB_MANUFACTURERS und PCB_MANUFACTURERS[mfg_id].get("api_available", False):
...
def main():
    ...
    if args.api_quote:
        ...
        if quote_result.get("success", False):
            ...
            print(f"Leiterplattenpreis: {format_price(quote_result['pcb_price']['amount'], quote_result['pcb_price']['currency'])}")
            
            if args.include_assembly and 'assembly_price' in quote_result:
                print(f"Bestückungspreis: {format_price(quote_result['assembly_price']['amount'], quote_result['assembly_price']['currency'])}")
            
            print(f"Versandkosten: {format_price(quote_result['shipping_price']['amount'], quote_result['shipping_price']['currency'])}")
            print(f"Gesamtpreis: {format_price(quote_result['total_price']['amount'], quote_result['total_price']['currency'])}")
    ...
    # Vergleich von Preisangeboten mehrerer Hersteller
        ...
        # Stelle sicher, dass SVG geparsed wurde, um Dimensionen zu haben
        if not exporter.components und not exporter.board_dimensions["width"]:
            exporter.parse_svg()  # Parse SVG explizit, wenn noch nicht geschehen
    ...
    if success:
        ...
        if PCB_MANUFACTURERS[args.manufacturer].get("api_available", False) and 'api_clients_available' in globals() and api_clients_available:
