...
class RP2040Emulator:
    ...
    def detect_object(self, input_image_path=None, simulated_score=None):
        ...
        # Ask if user wants to continue with another detection
        return self.continue_iteration()
    
    def continue_iteration(self):
        """Ask user if they want to continue with another detection"""
...
"""
Unit-Tests für das Hardware-Emulator-Modul.
"""
...
def test_camera_emulator():
    ...
    # Test Framerate-Begrenzung
    start = time.time()
    frame = camera.capture_frame()
    elapsed = time.time() - start
    assert elapsed >= camera.frame_time
    
    # Test Bildformat-Konfiguration
    camera.set_format(32, 32, rgb=False)
    frame = camera.capture_frame()
    assert frame.shape == (32, 32, 1)

def test_rp2040_emulator():
    """Testet den RP2040-Emulator."""
    emulator = RP2040Emulator()
    
    # Test Initialisierung
    assert emulator.flash_size_bytes == RP2040_FLASH_SIZE_KB * 1024
    assert emulator.ram_size_bytes == RP2040_RAM_SIZE_KB * 1024
    assert not emulator.firmware_loaded
    
    # Test Firmware-Laden
    test_firmware = Path("test_firmware.bin")
    emulator.load_firmware(test_firmware)
    assert emulator.firmware_loaded
    assert emulator.firmware['path'] == str(test_firmware)
    ...
    assert 0 <= result['inference_time'] <= 1.0
    assert isinstance(result['class_id'], int)
    assert 0 <= result['confidence'] <= 1.0
    
    # Test Ressourcennutzung
    ram_usage = emulator.get_ram_usage()
    flash_usage = emulator.get_flash_usage()
    
    assert ram_usage > emulator.system_ram_overhead
    assert flash_usage > 0
    assert flash_usage == emulator.firmware['size_bytes']

def test_system_stats():
    """Testet die Systemstatistik-Berichterstattung."""
    emulator = RP2040Emulator()
    emulator.load_firmware("test.bin")
    ...
    assert all(key in stats for key in [
        'uptime_seconds', 'ram_used_kb', 'ram_free_kb',
        'flash_used_kb', 'flash_free_kb', 'firmware_loaded',
        'last_inference_ms', 'camera_frames'
    ])
    
    assert stats['uptime_seconds'] > 0
    assert stats['ram_used_kb'] > 0
    assert stats['flash_used_kb'] > 0
...
def test_error_conditions():
    ...
    # Test: Inferenz ohne Firmware
    with pytest.raises(HardwareError):
        emulator.simulate_inference(np.zeros((32, 32, 3)))
    
    # Test: Kamera-Nutzung ohne Initialisierung
    with pytest.raises(HardwareError):
        emulator.camera.capture_frame()
    
    # Test: Ressourcenüberlauf
    emulator.firmware = {
        'path': 'large.bin',
        'size_bytes': RP2040_FLASH_SIZE_KB * 1024 * 2,  # 2x Flash-Größe
        'loaded_at': time.time()
    }
    emulator.firmware_loaded = True
    
    with pytest.raises(ResourceError):
        emulator.validate_resources()

def test_reset_functionality():
    """Testet Reset-Funktionalität."""
    emulator = RP2040Emulator()
    
    # Setup: System in genutztem Zustand
    emulator.load_firmware("test.bin")
    emulator.camera.initialize()
    emulator.camera.capture_frame()
    ...
    # Prüfe Reset-Zustand
    assert not emulator.firmware_loaded
    assert emulator.firmware is None
    assert emulator.inference_time == 0
    assert not emulator.camera.initialized
    assert emulator.camera.frames_captured == 0
    
    # Prüfe dass Statistiken zurückgesetzt wurden
    new_stats = emulator.get_system_stats()
    assert new_stats['ram_used_kb'] < initial_stats['ram_used_kb']
    assert new_stats['flash_used_kb'] == 0
    assert new_stats['camera_frames'] == 0