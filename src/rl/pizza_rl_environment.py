...
from ..constants import CLASS_NAMES, INPUT_SIZE
...
try:
    ...
    RP2040Emulator = None  # type: ignore
    AdaptiveMode = None    # type: ignore
    logging.warning("RP2040 Emulator not available, RL environment will run in simulation mode")
...
try:
    from ..models.architectures import MicroPizzaNet, MicroPizzaNetV2, MicroPizzaNetWithSE
...
class PizzaRLEnvironment(gym.Env):
    ...
    def _init_emulator(self) -> None:
        ...
        if not EMULATOR_AVAILABLE:
            logger.warning("Emulator not available, skipping initialization")
            self.emulator = None
            return
        ...
        try:
            ...
            # Load default firmware for pizza detection
            default_firmware = {
                'path': 'pizza_detector_v2.bin',
                'total_size_bytes': 128 * 1024,
                'model_size_bytes': 85 * 1024,
                'ram_usage_bytes': 60 * 1024,
                'model_input_size': (INPUT_SIZE, INPUT_SIZE),
                'pixel_format': 'RGB565'
            }
            self.emulator.load_firmware(default_firmware)
            
            logger.info("RP2040 emulator initialized successfully")
        except Exception as e:
            logger.error(f"Failed to initialize real emulator: {e}")
            raise e
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        if task is None:
            # Fallback if no task could be selected
            logger.warning("No task available for observation, using default values")
    ...
    def _execute_inference(self, strategy: InferenceStrategy) -> ActionResult:
        ...
        if self.emulator is not None and task.image_data is not None:
            # Use real emulator
    ...
    def get_episode_summary(self) -> Dict[str, Any]:
        ...
        acc_scores = cast(List[float], metrics['accuracy_scores'])
        inf_times = cast(List[float], metrics['inference_times'])
        ver_quals = cast(List[float], metrics['verifier_qualities'])
        
        summary = {
            'total_steps': self.current_step,
            'total_reward': metrics['total_reward'],
            'average_accuracy': float(np.mean(acc_scores)) if acc_scores else 0.0,
            'total_energy_consumed': metrics['energy_consumed'],
            'average_inference_time': float(np.mean(inf_times)) if inf_times else 0.0,
            'average_verifier_quality': float(np.mean(ver_quals)) if ver_quals else 0.0,
...
class PizzaRLEnvironmentFactory:
    ...
    def create_training_environment(
        ...
        # Filter config to only include valid arguments for PizzaRLEnvironment
        valid_keys = [
            'max_steps_per_episode', 'initial_battery_level', 
            'task_dataset_path', 'reward_weights'
        ]
