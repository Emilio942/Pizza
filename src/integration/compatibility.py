...
from src.models.architectures import MicroPizzaNet, MicroPizzaNetV2, MicroPizzaNetWithSE
...
class ModelCompatibilityManager:
    ...
    def _check_cmsis_availability(self) -> bool:
        ...
        try:
            # Check for CMSIS-NN related files
            cmsis_paths = [
            ...
            for path in cmsis_paths:
                if Path(path).exists():
                    # Also check for the header file if the C file exists
                    header_path = Path(path).parent / "pizza_model_cmsis.h"
                    if header_path.exists():
                        logger.info(f"CMSIS-NN export found: {path} and header {header_path}")
                        files_exist = True
                        break
            ...
            # Check for compiler availability (optional but good for full verification)
            compiler_path = shutil.which("arm-none-eabi-gcc")
            if compiler_path:
                logger.info(f"ARM GCC compiler found at {compiler_path}")
                return True
            else:
                logger.warning("CMSIS-NN files found but ARM GCC compiler not available")
                # We return True here because the files are present, so "integration" is possible
                # even if we can't compile on this specific machine right now.
                return True
    ...
    def check_model_compatibility(self, model_type: str) -> bool:
        """
        Check if a model type is compatible with the system.
        ...
        """
        return self._check_model_cmsis_compatibility(model_type)
...
class VerifierIntegration:
    ...
    def _check_formal_verification(self) -> bool:
        ...
        try:
            from src.models.formal_verification.formal_verification import ModelVerifier
            return True
        except ImportError:
    ...
    def integrate_with_formal_verification(
        ...
        if self.formal_verification_available:
            try:
                ...
                model = self.compatibility_manager.load_model(
                    model_type=model_type
                )
                
                # Load and preprocess image
                image_path = verifier_data.pizza_image_path
                if Path(image_path).exists():
                    try:
                        ...
                        formal_result = formal_verifier.verify(image_tensor)
                        ...
                        logger.warning(f"Failed to load image for verification: {img_err}")
                else:
                    logger.warning(f"Image not found for verification: {image_path}")
